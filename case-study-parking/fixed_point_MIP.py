...
def getModel(data):
    ...
    # (constraints not needed, but they might improve the linear relaxation)
    '''
    for k in range(1, data['K'] + 1):
                ...
                for r in range(data['R']):
                    if data['w_pre'][i,n,r] != 0 and data['w_pre'][i,n,r] != 1:
                        ...
                        if k > 0:
                            for l in data['list_strategies_opt'][k]:
                                ...
                                if data['DCM'] == 'MixedLogit':
                                    if data['RESIDENT'][n] == 1 and i == 2:
                                        co.append(-data['endo_coef'][i, n, r] * (1-data['disc_residents_PUP']) * data['scenarios']['prices'][i][l])
                                    else:
                            ...
                            # Add right-hand side according to the used DCM
                            if data['DCM'] == 'MixedLogit':
                                rhsConstr.append(data['endo_coef'][i,n,r] * data['price'][i] + data['exo_utility'][i,n,r] + data['xi'][i,n,r])
                        indicesConstr.append(ind)
                        coefsConstr.append(co)
                        sensesConstr.append('E')
...
def solveModel(model, nameToIndex, data):
    ...
    for k in range(1, data['K'] + 1):
        for i in data['list_alt_supplier'][k]:
            results['max_profit_in'][k] += results['demand'][i] * (results['prices_in'][i] - data['customer_cost'][i])
            results['max_profit_fin'][k] += results['demand_fin'][i] * (results['prices_fin'][i] - data['customer_cost'][i])
